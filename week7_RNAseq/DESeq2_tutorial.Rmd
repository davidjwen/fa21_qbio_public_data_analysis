...
* Use `GDCquery()`.
* Load the HDF5Array file from the RNA-seq tutorial using the `HDF5Array::loadHDF5SummarizedExperiment()`. This saves querying time!
...
You should see that over half of the genes are very lowly expressed. Let's prune these rows from `counts`, since they're just going to make our analysis slower without really adding much information.
...
results = DESeq2::results(dds_obj, format = "DataFrame", contrast = c("age_category", "young", "old"))
```
...
The most common way to visualize results is to plot each gene as a *volcano plot*. A volcano plot shows the expression of each gene, with the negative log10 (adjusted) p-value on the vertical axis and the log2FoldChange on the x-axis. Here's how to make one using `ggplot` (this might be hard to understand at first glance):
...
volcano_plot = ggplot(data = data.frame(results), aes(x = log2FoldChange, y = -log10(padj))) + 
  ...
  geom_vline(xintercept=c(-fc_threshold, fc_threshold), color="red") + 
  geom_hline(yintercept=-log10(p_threshold), color="red")

volcano_plot
```

You can play around with `ggplot` and the `results` objects to get more informative plots, but this should be good enough to get started. (Feel free to ask if you want to get fancier plots).
